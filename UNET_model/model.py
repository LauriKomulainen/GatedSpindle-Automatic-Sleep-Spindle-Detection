...
from UNET_model.ademamix import AdEMAMix
...
def train_model(model, train_loader, val_loader, optimizer_type, learning_rate, num_epochs, early_stopping_patience,
    ...
    if optimizer_type == 'Adam':
        ...
        # Haetaan AdEMAMix-spesifiset arvot
        alpha = TRAINING_PARAMS.get('ademamix_alpha', 5.0)
        betas = TRAINING_PARAMS.get('ademamix_betas', (0.9, 0.999, 0.9999))

        optimizer = AdEMAMix(model.parameters(), lr=learning_rate, weight_decay=weight_decay,
                             alpha=alpha, betas=betas)

        log.info(f">>> Using AdEMAMix (alpha={alpha}, betas={betas}, wd={weight_decay}) <<<")
